...
struct ConversationView: View, Sendable {
	...
	func submit(_ input: String) async {
		...
		do {
			_ = try await Message.create(text: input, fromId: Message.USER_SPEAKER_ID, conversation: conversation, systemPrompt: systemPrompt, inContext: viewContext)
		} catch (let error) {
	...
	func onUpdate(text: String) {
		...
		if newText.count > 100 || newText.isEmpty {
			...
			if conversationController.readAloud {
				readAloud(fullText: fullText, prevProgress: prevProgress)
			}
			// Reset buffer
	...
	func readAloud(fullText: String, prevProgress: String) {
		// Define utterance
